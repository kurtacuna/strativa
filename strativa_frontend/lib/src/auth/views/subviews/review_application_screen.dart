...
class ReviewApplicationScreen extends StatefulWidget {
  ...
  const ReviewApplicationScreen({Key? key, required this.userData}) : super(key: key);
...
class _ReviewApplicationScreenState extends State<ReviewApplicationScreen> {
  ...
  void initState() {
    ...
    fullNameController = TextEditingController(
      text: "${widget.userData['first_name']} ${widget.userData['middle_name']} ${widget.userData['last_name']}"
          .trim()
          .replaceAll(RegExp(r'\s+'), ' '),
    );

    birthdayController = TextEditingController(text: widget.userData['date_of_birth'] ?? '');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(isEditing ? Icons.save : Icons.edit, color: Colors.orange),
            onPressed: () {
              setState(() {
                isEditing = !isEditing;
              });
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text('Review your application', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            ...
            _buildEditableInfoSection([
              ...
              {'label': 'Marital Status', 'controller': maritalStatusController},
            ]),
  ...
  Widget _buildEditableInfoSection(List<Map<String, dynamic>> fields) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: fields.map((field) => _buildEditableTile(field['label'], field['controller'])).toList(),
      ),
    );
  }
